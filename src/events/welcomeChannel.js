...
module.exports = class extends Event {
    ...
    run(member) {
        member.guild.channels.find(c => c.id == member.guild.settings.welcomeLeaveChannel).send(new MessageEmbed().setAuthor(member.user.tag, member.user.avatarURL()).setColor('GREEN').setTitle(randomMessages[Math.floor(Math.random() * randomMessages.length)].replace("{guildname}", member.guild.name).replace("{tag}", member.user.tag)).setDescription(description.replace("{guildname}", member.guild.name).replace("{tag}", member.user.tag).replace("{membercount}", member.guild.memberCount)));
    }
